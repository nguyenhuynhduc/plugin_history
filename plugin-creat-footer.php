...
class CheckLogin
{
    ...
    public function  __construct()
    {
      ...
      //  do_action( 'my_action',array($this,'is_user_logged_in') );
        ...
        add_action( 'edit_user_profile_update', array($this,'editUser') );
    ...
    function addUser( $user_id ){
        ...
        $wpdb->insert($table_check_logs, array('id_user' => $current_user->ID, 'time_logs'=>$blogtime,'type'=>'user','user_login_log'=>$user_login,'user_action' => 'inserted'));
    }
    function editUser( $user_id )
    {
        ...
        $blogtime = current_time ('mysql');
        $user_login=$wpdb->get_var("SELECT user_login FROM $table_user WHERE ID=$user_id");
        $wpdb->insert($table_check_logs, array('id_user' => $current_user->ID, 'time_logs'=>$blogtime,'type'=>'user','user_login_log'=>$user_login,'user_action' => 'updated'));
    }
    ...
    function deleteUser( $user_id ) {
        ...
        $blogtime = current_time ('mysql');
        $user_login=$wpdb->get_var("SELECT user_login FROM $table_user WHERE ID=$user_id");
        $wpdb->insert($table_check_logs, array('id_user' => $current_user->ID, 'time_logs'=>$blogtime,'type'=>'user','user_login_log'=>$user_login,'user_action' => 'deleted'));
    }
    function check_edit_post($post_id, $post, $update )
    {
        if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) { // keine Aktion bei Autosave
            ...
            if ($update)
            {
                ...
                $type=$wpdb->get_var('SELECT post_type FROM '.$table_posts.' WHERE ID='.$post_id);
                    if ($check_post==null)
                    {
                        $wpdb->insert($table_check_logs, array('id_user' => $current_user->ID,'id_post'=>$post_id,'type'=>$type, 'time_logs'=>$blogtime,'user_action' => 'inserted'));
                    }
                    else{
                        $check_delete =$wpdb->get_results('SELECT * FROM '.$table_posts.' WHERE ID='.$post_id.' AND post_status="trash";');
                        if ($check_delete!=null)
                        {
                            $wpdb->insert($table_check_logs, array('id_user' => $current_user->ID, 'id_post'=>$post_id,'type'=>$type,'time_logs'=>$blogtime,'user_action' => 'deleted'));

                        }
                        else{
                            $wpdb->insert($table_check_logs, array('id_user' => $current_user->ID, 'id_post'=>$post_id,'type'=>$type,'time_logs'=>$blogtime,'user_action' => 'updated'));
                        }
                    }
            }
     }
...
//export Excel
    function excel_export() {
        ...
        $table_user=$wpdb->prefix ."users";
        $table_posts=$wpdb->prefix ."posts";
        $logs = $wpdb->get_results("SELECT * FROM $table_check_logs 
        ...
        foreach ( $logs as $item ) {
            ...
            {
                $table_comments=$wpdb->prefix ."comments";
                $posts=$wpdb->get_results("SELECT * FROM $table_comments 
                 INNER JOIN $table_posts ON $table_posts.ID = $table_comments.comment_post_ID 
                WHERE comment_ID=$item->id_comment");
                foreach ($posts as $post)
                $action="Have ".$item->user_action." a ".$item->type." in ".$post->post_title.": ".$post->comment_content;
            }
            elseif($item->type=="log"){
                $action="Have ".$item->user_action;
            }
            elseif ($item->type=="user")
            {
                $action="Have ".$item->user_action." a ".$item->type." in user login: ".$item->user_login_log;
            }
            else{
                $post_title=$wpdb->get_var("SELECT post_title FROM $table_posts WHERE ID=$item->id_post");
                $action= " have " . $item->user_action."  a  ".$item->type.":  " .$post_title;
            }
    ...
    function  checkLogin($login) {
        ...
        $blogtime = current_time ('mysql');
        $wpdb->insert($table_check_logs, array('id_user' => $user->ID,'type'=>'log','time_logs'=>$blogtime,'user_action' => 'Login'));
    }
    function checkLogout()
    {
        ...
        $current_user = wp_get_current_user();
        $blogtime = current_time('mysql');
        $wpdb->insert($table_check_logs, array('id_user' => $current_user->ID,'type'=>'log', 'time_logs' => $blogtime, 'user_action' => 'Logout'));
    }
